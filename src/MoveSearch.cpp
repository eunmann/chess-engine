  ...
  auto get_best_move(const GameState& game_state) noexcept -> Move {
    ...
    for (auto& move : moves) {
      ...
      if (check.is_legal()) {
        ...
        if (color_to_move == Colors::WHITE) {
          int32_t heuristic = alpha_beta_pruning_search<Colors::BLACK>(check, max_search_depth, PieceValues::NEG_INFINITY, PieceValues::POS_INFINITY);
          if (best_heuristic < heuristic) {
            best_heuristic = heuristic;
            best_move = move;
          }
        } else {
          int32_t heuristic = alpha_beta_pruning_search<Colors::WHITE>(check, max_search_depth, PieceValues::NEG_INFINITY, PieceValues::POS_INFINITY);
          if (best_heuristic > heuristic) {
            best_heuristic = heuristic;
            best_move = move;
          }
