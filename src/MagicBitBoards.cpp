...
namespace MagicBitBoards {
    ...
    auto get_bishop_moves(const Square square, const BitBoard blockers) noexcept -> BitBoard {
        ...
        const uint64_t index = (occupied_blocker * BISHOP_MAGICS.magics[square])
                >> (Squares::NUM - BitBoardUtils::get_count(BISHOP_MAGICS.blocker_masks[square]));
    ...
    auto get_rook_moves(const Square square, const BitBoard blockers) noexcept -> BitBoard {
        ...
        const uint64_t index = (occupied_blocker * ROOK_MAGICS.magics[square])
                >> (Squares::NUM - BitBoardUtils::get_count(ROOK_MAGICS.blocker_masks[square]));
