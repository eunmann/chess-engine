...
namespace MoveSearch {
    ...
    auto alpha_beta_pruning_search(const GameState &game_state, const int32_t ply_depth, int32_t alpha,
        ...
        for (auto &move: moves) {
            ...
            if constexpr (max_color == Colors::WHITE) {
                best_heuristic = std::max(best_heuristic,
                                          MoveSearch::alpha_beta_pruning_search<opponent_color>(check, ply_depth - 1,
                                                                                                alpha, beta));
