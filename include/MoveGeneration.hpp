...
constexpr auto get_pawn_moves(const GameState& game_state, Moves& moves)
  ...
  GameUtils::for_each_bit_board(pawns_bit_board, [&game_state, &moves,
                                                  pawn_dir](
                                                     const BitBoard
                                                         pawn_bit_board) {
    Square source_square = GameUtils::bit_board_to_square(pawn_bit_board);

    auto add_promotion_moves = [source_square,
                                &moves](const BitBoard destination_bit_board) {
...
constexpr auto get_knight_moves(const GameState& game_state, Moves& moves)
  ...
  BitBoard knights_bit_board =
      game_state.position.get_piece_color_bit_board(PieceCodes::KNIGHT, color);
...
constexpr auto get_bishop_moves(const GameState& game_state, Moves& moves)
  ...
  BitBoard bishops_bit_board =
      game_state.position.get_piece_color_bit_board(PieceCodes::BISHOP, color);
    ...
    if constexpr (color == Colors::WHITE) {
      ...
      if (!game_state.white_king_moved && !game_state.is_white_in_check()) {
        ...
        if (!game_state.white_rook_A_moved &&
            ...
            !game_state.position.is_black_threaten(
      ...
      if (!game_state.black_king_moved && !game_state.is_black_in_check()) {
        ...
        if (!game_state.black_rook_A_moved &&
            ...
            !game_state.position.is_white_threaten(
...
constexpr auto get_rook_capture_positions(const Position& position)
  ...
  GameUtils::for_each_bit_board(rooks_bit_board, [&capturable_bit_board,
                                                  &position](
                                                     BitBoard rook_bit_board) {
