...
namespace MagicBitBoards {
  ...
  constexpr auto generate_blocker_board_permutation(const Square square, const BitBoard blocker_mask) noexcept -> BitBoard {
    ...
    std::for_each(index_checks.begin(), index_checks.end(), [&blocker_board, &bit_index, square, blocker_mask](auto index) {
  ...
  constexpr auto generate_bishop_magics() noexcept -> BishopMagics {
    ...
    for (Square square = Squares::A1; square <= Squares::H8; square++) {
      ...
      const BitBoard blocker_mask = CapturesGeneration::get_bishop_capture_positions(bishop_bit_board, BitBoards::EMPTY) & ~BitBoards::EDGES;
      ...
      for (int32_t i = 0; i < num_of_permutations; i++) {
        ...
        bishop_attacks[i] = CapturesGeneration::get_bishop_capture_positions(bishop_bit_board, bishop_blockers_permutations[i]);
      }
      ...
      while (true) {
        ...
        for (int32_t i = 0; i < num_of_permutations; i++) {
          ...
          const uint64_t magic_index = (bishop_blockers_permutations[i] * magic) >> (Squares::NUM - blocker_mask_bit_count);
  ...
  constexpr auto generate_bishop_magics_cached() noexcept -> BishopMagics {
    ...
    for (Square square = Squares::A1; square <= Squares::H8; square++) {
      ...
      for (int32_t i = 0; i < num_of_permutations; i++) {
        // Calculate the magic index into the move array
        const uint64_t magic_index = (bishop_blockers_permutations[i] * BISHOP_MAGICS_CACHE[square]) >> (Squares::NUM - blocker_mask_bit_count);
        bishop_magics.moves[square][magic_index] = bishop_attacks[i];
      }
    }
  ...
  constexpr auto generate_rook_magics() noexcept -> RookMagics {
    ...
    for (Square square = Squares::A1; square <= Squares::H8; square++) {
      ...
      blocker_mask |= CapturesGeneration::get_captures_in_direction<1, 0>(rook_bit_board, BitBoards::EMPTY) & ~BitBoards::ROW_8;
      blocker_mask |= CapturesGeneration::get_captures_in_direction<-1, 0>(rook_bit_board, BitBoards::EMPTY) & ~BitBoards::ROW_1;
      blocker_mask |= CapturesGeneration::get_captures_in_direction<0, -1>(rook_bit_board, BitBoards::EMPTY) & ~BitBoards::COL_A;
      blocker_mask |= CapturesGeneration::get_captures_in_direction<0, 1>(rook_bit_board, BitBoards::EMPTY) & ~BitBoards::COL_H;
      ...
      for (int32_t i = 0; i < num_of_permutations; i++) {
        ...
        rook_attacks[i] = CapturesGeneration::get_rook_capture_positions(rook_bit_board, rook_blockers_permutations[i]);
      }
      ...
      while (true) {
        ...
        for (int32_t i = 0; i < num_of_permutations; i++) {
          // Calculate the magic index into the move array
          const uint64_t magic_index = (rook_blockers_permutations[i] * magic) >> (Squares::NUM - blocker_mask_bit_count);
  ...
  constexpr auto generate_rook_magics_cached() noexcept -> RookMagics {
    ...
    for (Square square = Squares::A1; square <= Squares::H8; square++) {
      ...
      for (int32_t i = 0; i < num_of_permutations; i++) {
        // Calculate the magic index into the move array
        const uint64_t magic_index = (rook_blockers_permutations[i] * ROOK_MAGICS_CACHE[square]) >> (Squares::NUM - blocker_mask_bit_count);
        rook_magics.moves[square][magic_index] = rook_attacks[i];
      }
    }
