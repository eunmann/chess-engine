...
public:
    ...
    auto start_search(const GameState &game_state, uint32_t search_depth) noexcept -> void;
...
private:
    ...
    auto alpha_beta_pruning_search(const GameState &game_state, const int32_t ply_depth, int32_t alpha, int32_t beta,
                                   SearchResult &search_result) noexcept -> int32_t {
        ...
        for (auto &move: moves) {
            ...
            if constexpr (max_color == Colors::WHITE) {
                best_heuristic = std::max(best_heuristic,
                                          this->alpha_beta_pruning_search<opponent_color>(check, ply_depth - 1, alpha,
                                                                                          beta, search_result));
                ...
                best_heuristic = std::min(best_heuristic,
                                          this->alpha_beta_pruning_search<opponent_color>(check, ply_depth - 1, alpha,
                                                                                          beta, search_result));
    ...
    [[nodiscard]] auto should_search() const noexcept -> bool;
